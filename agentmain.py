...
if __name__ == '__main__':
    ...
    if _unknown and args.reflect is None:
        parser.error(f"unrecognized arguments: {' '.join(_unknown)}")
    if args.reflect is not None and (len(_unknown) % 2 or any(len(k) <= 2 or not k.startswith('--') or k.startswith('---') for k in _unknown[::2]) or any(v.startswith('--') for v in _unknown[1::2])):
